...
namespace arterm::ui
{
	...
	HostKeyDialog::HostKeyDialog( ssh::HostKeyInfo const& info, QWidget* parent )
		: QDialog( parent )
	{
		...
		body->setText( changed ? tr( "The key this server presented is different from the one stored in "
		...
		QFont const monospace = QFontDatabase::systemFont( QFontDatabase::FixedFont );

		auto const add_row = [&]( QString const& label, QString const& value ){
		...
		add_row( tr( "Host" ), QStringLiteral( "%1:%2" ).arg( info.hostname ).arg( info.port ) );
		add_row( tr( "Key type" ), info.key_type );
		add_row( tr( "SHA256" ), info.sha256 );
		if( !info.md5.isEmpty() )
			add_row( tr( "MD5" ), info.md5 );
	...
	bool HostKeyDialog::ask( ssh::HostKeyInfo const& info, QWidget* parent ){
