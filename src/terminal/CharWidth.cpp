...
constexpr std::array kZeroWidth = std::to_array<Range>({
    ...
    {0x10A01, 0x10A0F}, {0x10A38, 0x10A3F}, {0x11000, 0x11002},
    {0x11038, 0x11046}, {0x1107F, 0x11082}, {0x110B0, 0x110BA},
    {0x11100, 0x11102}, {0x11127, 0x11134}, {0x11180, 0x11182},
    {0x111B3, 0x111C0}, {0x1122C, 0x11237}, {0x112DF, 0x112EA},
    {0x11301, 0x11303}, {0x1133C, 0x1134D}, {0x11362, 0x11374},
    ...
    {0x1D242, 0x1D244}, {0x1DA00, 0x1DA8B}, {0x1E8D0, 0x1E8D6},
    {0xE0100, 0xE01EF},
});

/// East Asian Wide and Fullwidth ranges, plus the emoji blocks terminals render
/// double width.
constexpr std::array kDoubleWidth = std::to_array<Range>({
...
constexpr bool contains(const std::array<Range, N> &ranges, char32_t codePoint) noexcept
{
    const auto it = std::upper_bound(ranges.begin(), ranges.end(), codePoint,
                                     [](char32_t value, const Range &range) { return value < range.first; });
    if (it == ranges.begin())
        return false;
    const Range &candidate = *std::prev(it);
    return codePoint >= candidate.first && codePoint <= candidate.last;
}

} // namespace

int characterWidth(char32_t codePoint) noexcept
{
    if (codePoint == 0)
        return 0;
    ...
    if (contains(kDoubleWidth, codePoint))
        return 2;
